...
window.config = {
  ...
  whiteLabeling: {
    createLogoComponentFn: function (React) {
      var user = null;
      try {
        user = JSON.parse(localStorage.getItem('dental.user') || 'null');
      } catch (e) {
        user = null;
      }
      var practice = (user && user.practiceName) || 'Practice Dental';
      return React.createElement(
        ...
        React.createElement(
          ...
          React.createElement('div', { style: { fontWeight: 600, fontSize: '15px', color: '#fff' } }, practice),
          React.createElement(
            'div',
            { style: { fontSize: '11px', color: '#5ac5b4' } },
            'OHIF Dental Mode'
          )
        )
      );
